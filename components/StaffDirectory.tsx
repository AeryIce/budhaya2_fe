...
export function StaffDirectory() {
  ...
  const selected = staffMembers.find((item) => item.id === selectedId) ?? null;
      ...
      <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
        {staffMembers.map((member) => (
          ...
          >
            ...
            <div className="p-5">
              <div className="text-xs font-semibold uppercase tracking-[0.16em] text-sky-700">Karyawan Sekolah</div>
              <div className="mt-2 text-lg font-semibold text-slate-900">{member.name}</div>
              <div className="mt-1 text-sm font-medium text-slate-700">{member.role}</div>
              <div className="mt-3 text-sm leading-7 text-slate-600">{member.shortBio}</div>
            </div>
          </button>
        ))}
      </div>
      ...
      {selected ? (
        <div className="fixed inset-0 z-[90] flex items-center justify-center p-4 md:p-6">
          ...
          <div
            ...
            className="relative z-[91] max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-[32px] border border-white/40 bg-white shadow-[0_30px_100px_-35px_rgba(15,23,42,0.6)]"
          >
            <div className="flex items-start justify-between gap-4 border-b border-sky-100 bg-sky-50/80 px-5 py-4 md:px-7">
              <div>
                <div className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-700">Detail Karyawan — Mockup Popup</div>
                <div id="staff-detail-title" className="mt-2 text-2xl font-semibold tracking-tight text-slate-900">
                  {selected.name}
                </div>
              ...
              <div className="grid gap-6 lg:grid-cols-[0.72fr_1.28fr]">
                ...
                <div className="grid gap-4">
                  <div className="rounded-[24px] bg-sky-50 p-5">
                    <div className="text-sm font-semibold text-sky-900">Profil Singkat</div>
                    <div className="mt-3 text-sm leading-7 text-slate-700">{selected.shortBio}</div>
                  </div>

                  <div className="rounded-[24px] border border-dashed border-sky-200 bg-white p-5 text-sm leading-7 text-slate-500">
                    Data detail bidang tugas, jabatan, maupun informasi pendukung karyawan tetap bisa disesuaikan lagi
                    mengikuti keputusan resmi dari pihak sekolah.
                  </div>
                </div>
              </div>
            </div>
